...
sorted_clustering = sorted(clustering.items(),key=itemgetter(1),reverse=True) # least central ones as seeds
...
for i in range(50):
    for j in range(int(num_seeds/2)):
        seeds[i,j] = sorted_clustering[i*num_seeds+j][0]
    for j in range(int(num_seeds/2),num_seeds):
        seeds[i,j] = random.choice([k for k in range(len(sorted_clustering)) if k not in seeds[i,:]]) 
'''
seeds = np.zeros((50,num_seeds))
