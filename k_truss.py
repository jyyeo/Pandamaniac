...
def get_seed_nodes(graph_data, n_players, n_seeds, n_rounds):
    ...
    cc = {x: nx.algorithms.centrality.closeness_centrality(G, x) for x in truss_nodes}
    truss_nodes_sorted = sorted(list(truss_nodes), key=lambda x: cc[x])
    ...
    for x in nodes:
        if x not in cc:
            cc[x] = nx.algorithms.centrality.closeness_centrality(G, x)

    nodes_sorted = sorted(nodes, key=lambda x: cc[x])
